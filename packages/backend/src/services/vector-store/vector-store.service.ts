...
import {EmbeddingsConfig, EmbeddingsService} from "../embeddings/embeddings.service";
...
export class VectorStoreService {
  ...
  async createVectorStore(config: VectorStoreConfig): Promise<VectorStore> {
    const embeddings = this.embeddingsService.createEmbedding(config.embeddingsConfig);
    switch (config.type) {
      case 'Pinecone':
        ...
        return PineconeStore.fromExistingIndex(
          ...
          {
            pineconeIndex,
            namespace: this.documentNamespaceService.getDocumentNamespace(config.document),
          },
        );
      default:
